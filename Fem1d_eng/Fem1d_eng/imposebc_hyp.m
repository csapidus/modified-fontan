function [uh,matout,bout,stima_cond]=imposebc_hyp(matfem,bfem,ndof,bc,massa,matold,bold,...
...
if infl==0
   %
   bfem = bfem - matfem(:,1)*bc;
   bout = bfem([2:ndof]);
   matout = matfem([2:ndof],[2:ndof]);
   aux = 1/DeltaT*massa(:,1)*bc;
   aux2 = 1/DeltaT*massa*u_old;
   %
   matrice = 1/DeltaT*massa([2:ndof],[2:ndof])+theta*matout;
   t_noto= aux2(2:ndof) + ...
      theta*bout + (1-theta)*(-matold*u_old(2:ndof)+bold) - aux(2:ndof);
%   format rat
%   massa([2:ndof],[2:ndof])
%  matold
%   format long
%   abs(eig(eye(ndof-1,ndof-1)-inv(1/DeltaT*massa([2:ndof],[2:ndof]))*matold))
%   pause
   %   
...
elseif infl==1
   %
   bfem = bfem - matfem(:,ndof)*bc;
   bout = bfem([1:ndof-1]);
   matout = matfem([1:ndof-1],[1:ndof-1]);
   aux = 1/DeltaT*(massa(:,ndof)*bc);
   aux2 = 1/DeltaT*massa*u_old;
   %
   matrice = 1/DeltaT*massa([1:ndof-1],[1:ndof-1])+theta*matout;
   t_noto= aux2(1:ndof-1) + ...
      theta*bout + (1-theta)*(-matold*u_old(1:ndof-1)+bold) - aux(1:ndof-1);
   %   
   [ufem]=fsolver(matrice,t_noto,types);
   uh = [ufem; bc];
elseif infl==2
...
%   t_noto=1/DeltaT*massa*u_old + theta*bout + (1-theta)*(matold*u_old(1:ndof-1)+bold);
%   %   
