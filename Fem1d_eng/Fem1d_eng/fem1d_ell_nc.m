function [uh,coord,out1,out2]=fem1d_ell(Omega,bc,femsys,coeff,hmesh,arg1,arg2,arg3)
%   
...
if get(FORMA(1),'Value')==1 then % froma conservativa
   [matfem, bfem] = assembla(fem,ne,ndof,nln,dof,list_el,coeff,'c ');
else
   [matfem, bfem] = assembla(fem,ne,ndof,nln,dof,list_el,coeff,'nc');
end;   
[uh]  = imposebc(matfem,bfem,ndof,bc,types);
coord = dof;
if get(USCITA(3),'Value')==1 then
   figure(4);
   spy(matfem);
   title('Pattern della matrice');
   pause
   close;
end;   
